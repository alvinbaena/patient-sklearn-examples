...
def find_new_explore_c(paramArray, best_val):
    ...
    distance_next = float('inf') if index_val == len(paramArray) - 1 else float(
        paramArray[index_val + 1] - paramArray[index_val])
    ...
    if index_val != 0 and index_val != len(paramArray) - 1:
        paramArray = np.linspace(paramArray[index_val - 1] + distance_prev / 2,
                                 paramArray[index_val + 1] - distance_next / 2, len(paramArray)).tolist()
